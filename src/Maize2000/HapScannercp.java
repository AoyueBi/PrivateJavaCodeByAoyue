...
public class HapScannercp {
    ...
    public void parseParameters (String infileS) {
        ...
        /*1.将taxaBamfile读进表格，建立一个空的HashSet,目的是将重复的taxa放进set里去，避免重复。
        for循环表格，取第1列taxa信息，加入taxaSet.
        taxaSet转化为String类型数组，数组大小是taxaSet的size，然后排序。
        for循环taxaArray数组,在循环里建一个bamList，说明一个taxa可能有一个bam也可能有2甚至多个bam文件（由于bam文件有重复）。
        将taxa和bam文件的映射关系，放在taxaBamMap中。key是taxa，value是bamList;
        此时，taxaBamMap中只有taxa也即是key值，还没有value值。
        
        for 循环啊，key是第1列taxa，value是taxaBamsMap的get(key),
        将2列bam路径加进bamList。更新taxaBamsMap的value值。   
        
        第一个for循环是为了建立taxa HashSet
        第二个for循环是为了将taxa放入taxaBamsMap中的key
        第三个for循环是为了将bam文件建立list并放进taxaBamsMap，完善taxaBamsMap
        */
